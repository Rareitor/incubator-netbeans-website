...
. Install Oracle's Java or Open JDK (v8, v9, v10, v11).
. Install Apache Ant 1.10 or greater (https://ant.apache.org/).

Once you're all set just enter the `netbeans` directory and type:

- `ant` to build the Apache NetBeans IDE on JDK 8, on JDK 9+ also type -Dpermit.jdk9.builds=true with the `ant` command.
